...
#define MYLOG(fmt, args...) do {\
	if(g_pstLog){\
		ConnSegvLog(g_pstLog, 2, "%s:%d(%s): SEGV: " fmt, __FILE__, __LINE__, __FUNCTION__ , ## args);\
	}\
} while(0);
...
static uint32_t ulSegvFreq = 0; // cnt/5s
...
int ConnSegvLog(SegvLogFile* pstLogFile, int iLogTime,const char* sFormat, ...)
{
	va_list ap;
...
static int CheckCnt()
{
	...
	if(ulSegvFreq >= 5000){ // 设的很大，相当于禁用频率限制
...
/*
 * 针对Segv打印的精简Log，不需要记录时间，如内存Map，栈回溯等，
 * 一方便可以减少gettimeofday的调用，少些不必要的信息，另一方面
 * 节约没一行的空间，看起来更清晰
 */
...
static int GetMemInfo(const char *pMapStr, MemInfo *pstMemInfo, const char *pPath, long lSP)
{
	...
	if (pMapping) { // 找到了进程路径字符串
		...
		/* 这种方法不支持多线程
		pPerm = strstr(pMapStr, "[stack]"); // 查找栈空间
		*/
		...
		if (pPerm) { // 找到
			...
			/*
			 * 下面是查找BSS段，查看SUSE机器，BSS段和Heap没有明确区分，BSS段在Heap的地址范围，二者重叠了
			 * 在tlinux下，BSS段和Heap则明确分开了，没有重叠，这里他们有一个共同特点：BSS段肯定是紧挨着
			 * 数据段，也就是数据段的结束地址等于BSS段的开始地址，根据这个特点来找BSS段的结束地址。
			...
			if (lTmp != 0 && pstMemInfo->qDataEndAddr == lTmp) { // 找到了BSS段:BSS段起始地址等于数据段的结束地址
				...
				// 组成32位地址字符串如0x12345678
				strncpy(sTmp, "0x", 3);
				// 格式如:0x12345678abcd-0x12345678ef00
				// 格式如:0x12345678-0x12345690
				pAddr++; // 跳过-
				// 这里64位可能没有12个字节，导致后面的地址非法，但是StrToNum可以自动去除
				strncat(sTmp, pAddr, PMAP_ADDR_LEN);
...
static void MySnprintf(int iLogTime, const char* sFormat, ...) //same action like  MYLOG or MYLOG2
{
	...
	if (iLogTime == 2) {
		...
		//now iDealSegvLogLeft > MAX_LINE_LENGTH
	...
	//1.  iRet < 0 or iRet >= iDealSegvLogLeft
	//or
	//2.  0 == iRet but iDealSegvLogLeft <150;
	if (iLogTime > 0) {  //MYPRINT same as MYLOG
		MYLOG2("%s\n",szDealSegvLog);
	}
	else {
		MYLOG2("%s",szDealSegvLog); //MYPRINT2 same as MYLOG2
	}
	InitMySnprintf();
}
...
static inline void CallSaveMe(long lSP)
{
	...
	do {
		...
		pstShm = AutoShmCreate(0x20120913, sizeof(SegvShm));
		if (!pstShm) break;
...
static void DoRestart()
{
	char sPath[64];
	char sLinkPath[512];
	char sMessage[64 + 1024];
	int iSock = -1;
	struct sockaddr_in stAddr;
	FILE *cmdline = NULL;
	int i = 0;

	snprintf(sPath, sizeof(sPath) - 1, "/proc/%d/cwd", getpid());
	memset(sLinkPath, 0, sizeof(sLinkPath));
	if(0 > readlink(sPath, sLinkPath, sizeof(sLinkPath))){
		MYLOG("cannot readlink");
	}
	snprintf(sMessage, sizeof(sMessage) - 1, "3:saveme:%s|", g_sCurrentWorkDir);
	if (strcmp(g_sCurrentWorkDir,sLinkPath)) {
		MYLOG("CWD Changed:init[%s] curr[%s]\n",g_sCurrentWorkDir,sLinkPath);
	}

	snprintf(sPath, sizeof(sPath) - 1, "/proc/%d/cmdline", getpid());
	cmdline = fopen(sPath, "rb");
	if (cmdline) {
		char *pArg = NULL;
		size_t iArgSize = 0;
		size_t iMessageLen = strlen(sMessage);

		while(getdelim(&pArg, &iArgSize, 0, cmdline) != -1)
		{
			int iRet = snprintf(sMessage + iMessageLen, sizeof(sMessage)- iMessageLen - 1, "%s ", pArg);
			if (iRet >= (int)(sizeof(sMessage)- iMessageLen - 1)) {
				break; //遇上超长的命令行了,没必要处理可能遇到的下一个参数
			}
			if (iRet < 0) {
				break; //出错也不继续处理.
			}
			iMessageLen = strlen(sMessage); //snprintf 保证至少有个'\0'
			if (0 == i) {
				sMessage[iMessageLen] = '|';
				++iMessageLen;
	...
	if ( (iSock = socket(AF_INET, SOCK_DGRAM, 0)) < 0){
		MYLOG("request to restart failed.");
	} else {
		SetNBlock(iSock);
		stAddr.sin_family = AF_INET;
		stAddr.sin_addr.s_addr = inet_addr("127.0.0.1");
		stAddr.sin_port = htons(g_wPort);
		sendto(iSock, sMessage, strlen(sMessage), 0, (struct sockaddr *)&stAddr, sizeof(stAddr));
		MYLOG("request to restart sent [%s] succeed.",sMessage);
	}
}
#endif
...
static void DealSegv(int iSigNo, siginfo_t * pstSigInfo, void * pContext)
{
	...
	if (CheckFlag() && CheckStub() && CheckCnt()) { //CONN接入组专用
		...
		FlushMySnprintf();
		MONITOR(73857); //按照SF2的API来上报

		//siglongjmp()有风险，须在可控条件下谨慎使用,只有非多线程才用
		siglongjmp(g__bEnv, 1);//jmp to main
		return;
	}
	...
	if (0 == disable_restart_on_core) {
		if(0 == g_iSendToDoRestartFlag) {
			...
			MONITOR(73856); //按照SF2的API来上报
                        /// spp/sf2 has self pull action,delete it
...
//pfCheckFlag为空则使用内部默认的CheckFlag_default().这个函数会查看pstAlbsDispatchShm.
		...
		unsigned long ulMainStackSP)
{
	...
	if(pstLog){
		g_pstLog = pstLog;
                ...
                InitHanleException();
		SegvInitLogFile(&g_stLog, GetFileName(2), 0, 5, 10000000);
		g_pstLog = &g_stLog;
	}
...
#ifndef  __cplusplus  //if not define C++
	//C++ 抛异常的时候,没有catch，默认会调用abort()再结束
	//gnu libc库中abort函数执行了比较复杂的动作，会导致segv没办法生成栈信息，
	//segv无法判断收到的SIGABRT是否来自异常或直接kill
	//因此暂时不考虑C++ 版本SIGABRT信号
	if(sigaction(SIGABRT, &stAct, NULL) < 0){
		perror("ABRT:sigaction error.");
		return -7;
	}
#endif
