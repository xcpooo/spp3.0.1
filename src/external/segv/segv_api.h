...
#define SEGV_VAL_STUB 0xaaaa5555 //桩值, 即二进制:1010101010101010010101010101

#define MAX_THREAD_NUM 10000
#define THREAD_HASH_MODES 9973
/*
 * 这里Data区和Stack区的大小可以调整，只需把下面
 * 二个宏修改即可，其他无需改动
 */
...
#define SEGV_TREAT(pfunc, plog) do{\
	...
	if(0 == sigsetjmp(g__bEnv, 1)){\
		...
		/* MONITOR(73858);*/\
		/*ConnSegvLog((plog), 2, const_cast<char *>("Restore from SEGV"));*/\
		ConnSegvLog((plog), 2, "Restore from SEGV");\
	}\
}while(0);
