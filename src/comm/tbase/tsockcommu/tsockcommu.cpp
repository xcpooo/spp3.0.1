...
void CTSockCommu::check_expire(void)
{
    ...
    {
        MONITOR(MONITOR_CONNSET_TIMER_CHECK);

        unsigned flow;
        list<unsigned> timeout_list;
        ix_->connset_->check_expire(now - expiretime_, timeout_list);

        CUSTOM_MONITOR(MONITOR_TIMER_CLEAN_CONN, timeout_list.size());
        //有超时回调
...
int CTSockCommu::poll(bool block)
{
    ...
    for (CEPollFlowResult::iterator it = result.begin(); it != result.end(); it++)
    {
        ...
        {
            ix_->connset_->dumpinfo(flow, LOG_NORMAL, "unknow event, closeconn flow");
            ix_->connset_->closeconn(flow);
...
int CTSockCommu::sendto(unsigned flow, void* arg1, void* arg2)
{
	...
	if (sppdll.spp_handle_report){
		CReport rpt;
		if (sppdll.spp_handle_report(flow, arg1, &rpt) == 0){

			rpt.set_local_addr(cc->_info.localip_, cc->_info.localport_);
			rpt.set_remote_addr(cc->_info.remoteip_, cc->_info.remoteport_);
			rpt.set_msg_cost(time_delay);
			
			int iret = rpt.do_report();
			if (iret != 0)
			{
    				INTERNAL_LOG->LOG_P_FILE(LOG_ERROR, "report ret:%d\n", iret);
			}
		}
	}
