...
public:
    ...
    //path:             返回日志文件绝对路径
    //buf_len:          path buf长度
    int get_log_ap_path_prefix(char *path, int buf_len);
    ...
    //app_id:	        APP ID
    //返回值:			0:成功  其它:失败
    int switch_net_log(const char *srv_name, const char *usr_name, const uint32_t app_id, const uint64_t uin, const uint32_t route_no);
    ...
    //转换日志级别为uls日志级别
    int switch_lvl_to_uls(int level);
    //打印格式化日志
