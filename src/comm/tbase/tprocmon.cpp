            ...
            {  // 热重启处理逻辑
                switch (reload_group->reload_)
                {
                    case PROC_RELOAD_START:
                    {
                        ...
                        do_fork(reload_group->basepath_, reload_group->exefile_, reload_group->etcfile_, reload_group->minprocnum_, reload_group->group_type_, reload_group->affinity_);
...
void CTProcMonSrv::kill_group(int grp_id, int signo)
{
    TProcObj* proc = NULL;
    TProcGroupObj* group = NULL;
    int i, j;
	map<int, int> proc_value;//老进程暂存

    for (i = 0; i < cur_group_; ++i)
    {
        group = &proc_groups_[i];

		if (group->groupinfo_.groupid_ == grp_id && group->curprocnum_ != GROUP_UNUSED)
		{
			for(j = 0; j < BUCKET_SIZE; ++j) {
				list_for_each_entry(proc, &group->bucket_[j], list_) {
					//先记录老进程，稍后删除，避免遍历过程中删除，影响遍历
...
bool CTProcMonSrv::do_event(int event, void* arg1, void* arg2)
{
    ...
    if (event & PROCMON_EVENT_PROCDOWN)
    {
        ...
        if(group->groupid_ != 0)
        {
            CUSTOM_MONITOR(MONITOR_WORKER_LESS, diff);
        }
        else
        {
            CUSTOM_MONITOR(MONITOR_PROXY_LESS, diff);
        } 
    ...
    if (event & PROCMON_EVENT_PROCDEAD)
    {
        ...
        // 如果worker进程依然存在，需要生成coredump文件
        if ((0 == kill(proc->procid_, 0)) || (errno != ESRCH))
        {
            if (_spp_g_exceptioncoredump)
            {
                GstackLog(proc->procid_);
                system("./coredelete.sh gstack 20");
                system("./coredelete.sh pkg 20");
            }
        }
        ...
        if (kill(proc->procid_, 0) == -1 && errno == ESRCH) //确信已经kill掉进程了才将fork新的进程.
        {
            ...
            do_fork(group->basepath_, group->exefile_, group->etcfile_, 1, group->group_type_, group->affinity_);     //fork a process
            if(group->groupid_ == 0)
            {
...
void CTProcMonSrv::do_fork(const char* basepath, const char* exefile, const char* etcfile, int num, unsigned group_type, unsigned mask)
{
    char cmd_buf[256] = {0};
...
void CTProcMonCli::run()
{
    ...
    while ((ret = commu_->recv(&msg_[1], msgtype)) > 0)
    {
        ...
        {
            TProcEvent* event = (TProcEvent*)msg_[1].msgcontent_;
            if (event->groupid_ != CLI_SEND_INFO(this)->groupid_)
            {
                INTERNAL_LOG->LOG_P_FILE(LOG_ERROR, "Please check:\n"
                                         "event group id:%d should be = client send info group id:%d\n",
                                         event->groupid_,
                                         CLI_SEND_INFO(this)->groupid_
                                        );
                exit(-1);
            }
...
void CTProcMonCli::exception_report(int signo)
{
    int ret;
    ...
    INTERNAL_LOG->LOG_P_FILE(LOG_FATAL, "Process exception, signo:%d, procid:%d.", signo, CLI_SEND_INFO(this)->procid_);

    ret = commu_->send(&msg_[0]);
    if (ret < 0)
    {
        INTERNAL_LOG->LOG_P_FILE(LOG_FATAL, "Failed to send exception, signo:%d, procid:%d.", signo, CLI_SEND_INFO(this)->procid_);
        return;
    }
