...
static void *map_file(const char *path, int size)
{
	...
	{
		printf("ftruncate map file error:%m\n");
		flock(fd, LOCK_UN);
		close(fd);
		return NULL;
	}
...
struct sq_head_t *sq_open(const char *file, int ele_size, int ele_count)
{
	if (!file || !ele_size || !ele_count)
	{
		return NULL;
	}
	...
	ele_size = (((ele_size + 7)>>3) << 3); // align to 8 bytes
	// We need an extra element for ending control
	long allocate_size = sizeof(struct sq_head_t) + SQ_NODE_SIZE_ELEMENT(ele_size)*(ele_count+1);
	// Align to 4MB boundary
	allocate_size = (allocate_size + (4UL<<20) - 1) & (~((4UL<<20)-1));
	printf("shm size needed for queue - %lu.\n", allocate_size);

	struct sq_head_t *shm =(struct sq_head_t *)map_file(file, allocate_size);
