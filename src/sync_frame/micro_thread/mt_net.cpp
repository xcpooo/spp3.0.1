...
void CSockLink::Close()
{
    ...
    // 2. 框架侦听释放
    MtFrame::Instance()->EpollDelObj(this);
...
int32_t CSockLink::CreateSock()
{
    ...
    // 加入epoll监控
    this->EnableInput();
    if (!MtFrame::Instance()->EpollAddObj(this))
    {
...
bool CSockLink::Connect()
{
    ...
    int32_t ret = mt_real_func(connect)(_fd, (struct sockaddr*)this->GetDestAddr(&addr), sizeof(struct sockaddr_in));
    if (ret < 0)
    {
        int32_t err = errno;
        if (err == EISCONN)
...
int32_t CSockLink::SendCacheUdp(void* data, uint32_t len)
{
    ...
    TAILQ_FOREACH_SAFE(item, &_wait_send, _link_entry, tmp)
    {
        ...
        int32_t ret = mt_real_func(sendto)(_fd, buff, buff_len, 0, 
    ...
    int32_t ret =  mt_real_func(sendto)(_fd, data, len, 0, 
                    (struct sockaddr*)this->GetDestAddr(&dst), sizeof(struct sockaddr_in));
    if (ret == -1)
    {
        if ((errno == EINTR) || (errno == EAGAIN) || (errno == EINPROGRESS))
...
int32_t CSockLink::SendData(void* data, uint32_t len)
{
    ...
    {
        this->EnableOutput();
        rc = MtFrame::Instance()->EpollCtrlAdd(_fd, EPOLLOUT); 
    }
    else
    {
        this->DisableOutput();
        rc = MtFrame::Instance()->EpollCtrlDel(_fd, EPOLLOUT); 
    }
...
int CSockLink::OutputNotify()
{
    ...
    {
        this->DisableOutput();
        if (!MtFrame::Instance()->EpollCtrlDel(_fd, EPOLLOUT))
        {
            MTLOG_ERROR("socket epoll mng failed[%m], wait timeout");
        }
    }
